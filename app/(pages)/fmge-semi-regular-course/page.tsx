...
import { fmgeSemiRegularCourse } from "../_data/cources/CourcesData";
import Link from "next/link";
import AOS from "aos";
import "aos/dist/aos.css";

export default function FMGESemiRegularCourse() {
  return (
    <>
      <main className="w-full overflow-hidden pb-24 bg-white text-gray-600">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold mb-6">
            {fmgeSemiRegularCourse.hero.title}
          </h1>

          <Link
            href={fmgeSemiRegularCourse.hero.cta.link}
            ...
            data-aos-delay="200"
          >
            {fmgeSemiRegularCourse.hero.cta.label} →
          </Link>
        </section>
        ...
        >
          <h2 className="text-3xl font-bold mb-4">
            {fmgeSemiRegularCourse.courseDetails.heading}
          </h2>

          <p className="text-gray-700 leading-relaxed mb-8">
            {fmgeSemiRegularCourse.courseDetails.description}
          </p>

          {/* HIGHLIGHTS */}
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {fmgeSemiRegularCourse.highlights.map((item, index) => (
              <li
                key={index}

...
import { fmgeSemiRegularOnlineCourse } from "../_data/cources/CourcesData";
import Link from "next/link";
import AOS from "aos";
import "aos/dist/aos.css";

export default function FMGESemiRegularOnlineCourse() {
  return (
    <>
      <main className="w-full overflow-hidden pb-24 bg-white text-gray-600">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold mb-6">
            {fmgeSemiRegularOnlineCourse.hero.title}
          </h1>

          <Link
            href={fmgeSemiRegularOnlineCourse.hero.cta.link}
            ...
            data-aos-delay="200"
          >
            {fmgeSemiRegularOnlineCourse.hero.cta.label} →
          </Link>
        </section>
        ...
        >
          <h2 className="text-3xl font-bold mb-4">
            {fmgeSemiRegularOnlineCourse.courseDetails.heading}
          </h2>

          <p className="text-gray-700 leading-relaxed mb-8">
            {fmgeSemiRegularOnlineCourse.courseDetails.description}
          </p>

          {/* HIGHLIGHTS */}
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {fmgeSemiRegularOnlineCourse.highlights.map((item, index) => (
              <li
                key={index}
